(function () {
  ...
  function getConfig() {
    ...
    return { cardW: 260, cardH: 380, sideOffset: 0, sideScale: 0.60, sideRotY: 30, farOffset: 0, farScale: 0.40, farRotY: 45, visible: 0 };
  }
  ...
  function applyTransforms(animate) {
    ...
    cards.forEach((card, i) => {
      ...
      let tx, ry, scale, opacity, blur, zIndex, pointerEvents;

      if (neg === 0) {
        tx = 0; ry = 0; scale = 1; opacity = 1; blur = 0; zIndex = 10; pointerEvents = 'all';
      } else if (neg === 1 || neg === -1) {
        const sign = Math.sign(neg);
        tx = sign * cfg.sideOffset; ry = sign * -cfg.sideRotY; scale = cfg.sideScale;
        opacity = 0.72; blur = 1.5; zIndex = 6; pointerEvents = 'all';
      } else if (neg === 2 || neg === -2) {
        const sign = Math.sign(neg);
        tx = sign * cfg.farOffset; ry = sign * -cfg.farRotY; scale = cfg.farScale;
        opacity = 0.35; blur = 4; zIndex = 3; pointerEvents = cfg.visible >= 2 ? 'all' : 'none';
      } else {
        const sign = neg > 0 ? 1 : -1;
        tx = sign * (cfg.farOffset + 120); ry = sign * -cfg.farRotY; scale = cfg.farScale * 0.7;
        opacity = 0; blur = 8; zIndex = 1; pointerEvents = 'none';
      }

      const dur = animate ? '0.72s' : '0s';
      card.style.transition = animate
        ? `transform ${dur} cubic-bezier(0.34,0.9,0.34,1), opacity ${dur} ease, filter ${dur} ease, z-index 0s`
        : 'none';
